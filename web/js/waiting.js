...
const COL_NAME = "Waiting";

const loadWaiting = () => {
  if (debug) console.log("loadWaiting");
  $("#list").html("");
  $("#list-boards").html("");

  $.when(getMyBoards()).then(function (data) {
    // if (debug) console.log("B: " + data.length + "");

    $.each(data, function (id, board) {
      // if (debug) console.log("B- " + board.id + " " + board.name);

      $.when(getNamedListFromBoard(board.id, COL_NAME, true)).then((list) => {
        if (debug) console.log("L-- " + list.id + " " + list.name);
...
const printCard = (card, board) => {
  ...
  var itemStr =
    `<li class="card show" data-sortkey="${
      itemCreationDate.getTime() / 1000
    ...
    "  <div class='badges'>" +
    `   <span class="badge date creation-date">${itemCreationDate.getFullYear()}-${
      itemCreationDate.getMonth() + 1
    }-${itemCreationDate.getDate()}</span>` +
    (card.due != null
      ? ` <span class="badge date due-date">${itemDueDate.getFullYear()}-${
          itemDueDate.getMonth() + 1
        }-${itemDueDate.getDate()}</span>`
      : "") +
...
var printCards = function () {
  ...
  $.each(tasks, function (id, item) {
    ...
    var itemStr =
      `<li class="${itemClass}">` +
      `<h2><a href='http://trello.com/c/${item.id}" target='_blank'>` +
      item.name +
      "</a></h2>" +
      "<div class='badges'>" +
      ...
      `</span>` +
      `<span class="badge board board-${item.idBoard}">` +
      getBoardName(item.idBoard) +
      `</span>` +
      ` <span class="badge list list-${item.idList}">` +
